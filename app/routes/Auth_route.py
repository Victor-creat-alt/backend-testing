...
@auth_bp.route('/users/verify-email', methods=['GET'])
...
def login():
    ...
    if user and user.check_password(password):  # Pass the plain-text password here
        ...
        print(f"Generated access token for user {user.email} with role {user.role}")  # Debug log
        response = {
            ...
            'user': {
                ...
                'email': user.email,
                'role': user.role,
                'userType': user.role  # Add userType for frontend routing
            },
            'redirect': '/dashboard' if user.role == 'Admin' else '/home'
        }
